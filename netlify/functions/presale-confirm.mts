...
function signerSet(tx: any) {
  ...
  for (const row of tx?.transaction?.message?.accountKeys || []) if (row?.signer) out.add(String(row?.pubkey?.toBase58?.() || row?.pubkey || ''));
  return out;
}
function tokenOwnerBalances(rows: any[] | null | undefined, mint: string) {
  ...
  for (const row of rows || []) {
    ...
    const owner = String(row.owner), amount = BigInt(row?.uiTokenAmount?.amount || '0');
    out.set(owner, (out.get(owner) || 0n) + amount);
  }
  return out;
}
function deltaFor(owner: string, pre: Map<string, bigint>, post: Map<string, bigint>) { return (post.get(owner) || 0n) - (pre.get(owner) || 0n); }
function hasExpectedMemo(tx: any, memo: string) {
  for (const ix of tx?.transaction?.message?.instructions || []) {
    const programId = String(ix?.programId?.toBase58?.() || ix?.programId || '');
    if ((programId === MEMO_PROGRAM || ix?.program === 'spl-memo') && JSON.stringify(ix).includes(memo)) return true;
  }
  return false;
}
...
export default async (req: Request) => {
  ...
  const quoteId = String(body?.quoteId || '').trim(), signature = String(body?.signature || '').trim();
  ...
  try {
    ...
    const receipt = await withMutationLock(s, async () => {
      ...
      if (Date.now() > Number(quote.expiresAtMs || 0) + CONFIRM_GRACE_MS) throw new Error('Quote confirmation window expired. If USDC was already sent, contact the project owner with the transaction signature for reconciliation.');
      ...
      for (const [owner, expected] of expectedCredits) if (deltaFor(owner, pre, post) !== expected) throw new Error(`USDC credit to ${owner} does not match the locked quote.`);
