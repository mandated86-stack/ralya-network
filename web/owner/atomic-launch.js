...
async function waitSig(sig,label){
  ...
  throw new Error(`${label} was broadcast but confirmation timed out. Do not retry until the public sale state is checked. Signature: ${sig}`);
}
...
async function verifyPausedState(p){
  ...
  if(!saleInfo||saleInfo.data.length<250)throw new Error('Sale state is unavailable after atomic activation.');
  ...
  const status=saleBytes[248],startedAt=i64(saleBytes,240),unlockAt=i64(lockBytes,80),released=Boolean(lockBytes[88]);
  if(status!==2)throw new Error(`Atomic activation did not finish PAUSED; on-chain status=${status}`);
  if(startedAt<=0n)throw new Error('Founder lock start timestamp was not set.');
  if(unlockAt-startedAt!==365n*24n*60n*60n)throw new Error('Founder lock is not exactly 365 days after activation.');
  if(released)throw new Error('Founder lock unexpectedly reports released=true.');
  return {startedAt:startedAt.toString(),unlockAt:unlockAt.toString()};
}
...
async function atomicActivateAndPause(){
  ...
  if(cfg.presaleEnabled)throw new Error('Public presale master switch must remain OFF during activation.');
  ...
  const activateIx=new TransactionInstruction({programId,keys:[
    ...
    {pubkey:saleVault,isSigner:false,isWritable:false},{pubkey:founderLock,isSigner:false,isWritable:true},{pubkey:founderVault,isSigner:false,isWritable:false},
  ...
  const verified=await verifyPausedState(p);
  ...
  p.founderLockStartedAt=verified.startedAt;p.founderUnlockAt=verified.unlockAt;
  saveProgress(p);
  ...
  log('ATOMIC ACTIVATION PASS. The founder 365-day clock started and the same Solana transaction finished with the public sale PAUSED. There was no inter-transaction ACTIVE window.');
...
document.addEventListener('click',(event)=>{
  ...
  if(!confirm('Start the 365-day founder lock and finish PAUSED atomically in one Solana transaction? Public buying stays closed.'))return;
...
document.addEventListener('click',(event)=>{
  ...
  if(target.closest('#downloadConfig')){
    const p=getProgress();if(!p||p.phase!=='activated-paused')return;
    event.preventDefault();event.stopImmediatePropagation();
    download('RALYA_MAINNET_SITE_VALUES.json',{project:'RALYA',network:'mainnet-beta',launchPhase:p.phase,presaleEnabled:false,rlyaMint:p.rlyaMint,saleProgramId:p.saleProgramId,salePda:p.salePda,treasuryWallet:p.treasuryWallet,usdcMint:p.usdcMint});
  }
},true);
