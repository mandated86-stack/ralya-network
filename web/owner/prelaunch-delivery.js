...
async function preflight(){
  if(!manifest) throw new Error('Load the owner-exported delivery manifest first.'); if(!owner) await connectOwner(); const a=addresses(); const info=await connection.getAccountInfo(a.sale,'confirmed'); if(!info)throw new Error('Production sale account not found.'); const sale=decodeSale(info.data);
  if(!sale.admin.equals(owner))throw new Error('Connected wallet is not the on-chain sale admin.'); if(!sale.mint.equals(a.mint))throw new Error('Sale mint mismatch.'); if(sale.status!==2)throw new Error('Sale must be PAUSED for pre-launch distribution.'); if(sale.presaleCap!==PRESALE_CAP)throw new Error('On-chain presale cap does not equal 100.68M RLYA.');
  const manifestTotal=BigInt(manifest.totals.totalRlyaBase); if(sale.totalSold+manifestTotal>PRESALE_CAP)throw new Error('Existing on-chain sold amount plus manifest would exceed the fixed presale cap.');
  const vault=await connection.getTokenAccountBalance(a.saleVault,'confirmed'); if(BigInt(vault.value.amount)<manifestTotal)throw new Error('Official sale vault does not hold enough RLYA for this manifest.');
  await ensureMetrics(a); log(`Distribution preflight PASS · ${manifest.allocations.length} wallets · ${fmtRlya(manifestTotal)} · prior on-chain sold ${fmtRlya(sale.totalSold)}.`); return a;
}
