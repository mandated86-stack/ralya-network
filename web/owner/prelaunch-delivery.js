...
function decodeMetrics(data){
  const b=data instanceof Uint8Array?data:new Uint8Array(data); if(b.length<153)throw new Error('Pre-launch metrics account is too small for the staking-aware schema.');
...
async function markPublicLaunch(){
  if(!owner)await connectOwner(); const a=addresses(); const sale=await readSale(a);
  ...
  const tx=new Transaction().add(new TransactionInstruction({programId:a.program,data:await dataNoArgs('mark_public_launch'),keys:[
    {pubkey:owner,isSigner:true,isWritable:false},{pubkey:a.mint,isSigner:false,isWritable:false},{pubkey:a.sale,isSigner:false,isWritable:true},{pubkey:a.founderLock,isSigner:false,isWritable:true}
  ]}));
  ...
  log(`DAY 0 locked on-chain: ${new Date(Number(updated.publicLaunchAt)*1000).toISOString()}. Standard presale release starts day 21; Buy + Stake release starts day 36. Founder one-year lock starts from the same launch timestamp.`);
}
...
function rowReleaseReady(row,sale,nowSec){
...
async function buildWalletTx(a,row,sale,nowSec){
  ...
  const needsWeb=web>0n && rowReleaseReady(row,sale,nowSec);
  const manualReady=manual>0n && sale.publicLaunchAt>0n && BigInt(nowSec)>=sale.publicLaunchAt+BigInt(STANDARD_RELEASE_SECONDS);
...
async function verifyManifestFile(file){
  ...
  if(parsed.project!=='RALYA'||parsed.symbol!=='RLYA'||parsed.purpose!=='prelaunch-allocation-delivery'||parsed.version!==2) throw new Error('Not a staking-aware RALYA pre-launch delivery manifest v2.');
...
async function pendingManifestState(a,sale){
  ...
  for(const row of manifest.allocations){
    ...
    if(web>0n){
      const [receipt]=PublicKey.findProgramAddressSync([enc.encode('prelaunch_delivery'),a.mint.toBytes(),recipient.toBytes()],a.program);
      if(await connection.getAccountInfo(receipt,'confirmed'))completedParts+=1;else{pendingBase+=web;pendingBonus+=bonus;pendingParts+=1;if(rowReleaseReady(row,sale,nowSec))eligibleParts+=1;else deferredParts+=1;}
    }
    if(manual>0n){
      const [receipt]=PublicKey.findProgramAddressSync([enc.encode('prelaunch_manual_delivery'),a.mint.toBytes(),recipient.toBytes()],a.program);
      if(await connection.getAccountInfo(receipt,'confirmed'))completedParts+=1;else{pendingBase+=manual;pendingParts+=1;if(sale.publicLaunchAt>0n&&BigInt(nowSec)>=sale.publicLaunchAt+BigInt(STANDARD_RELEASE_SECONDS))eligibleParts+=1;else deferredParts+=1;}
    }
  }
...
async function preflight(){
  ...
  log(`Distribution preflight PASS · launch ${new Date(Number(sale.publicLaunchAt)*1000).toISOString()} · pending base ${fmtRlya(pending.pendingBase)} · pending bonus ${fmtRlya(pending.pendingBonus)} · eligible parts now ${pending.eligibleParts} · deferred by release lock ${pending.deferredParts} · completed receipt parts ${pending.completedParts}.`);
  return {a,sale,pending};
}
async function runDistribution(){
  if(running)return; running=true; try{
    const {a,sale,pending}=await preflight();
    if(pending.eligibleParts===0){ log('No allocation is eligible for delivery yet. Standard rows unlock on day 21; Buy + Stake rows unlock on day 36.'); return; }
    ...
    for(let start=0;start<rows.length;start+=BATCH_SIZE){
      ...
      for(const row of batch){
        const web=BigInt(row.webRlyaBase||0),manual=BigInt(row.manualRlyaBase||0); const webReady=web===0n||rowReleaseReady(row,sale,nowSec); const manualReady=manual===0n||(sale.publicLaunchAt>0n&&BigInt(nowSec)>=sale.publicLaunchAt+BigInt(STANDARD_RELEASE_SECONDS));
        if(!webReady||!manualReady)deferred+=1;
        const tx=await buildWalletTx(a,row,sale,nowSec); if(tx){ const latest=await connection.getLatestBlockhash('confirmed'); tx.recentBlockhash=latest.blockhash; tx.feePayer=owner; tx.__ralyaBlockhash=latest; txs.push(tx); labels.push(row.wallet); } else skipped+=1;
      }
    ...
    const after=await pendingManifestState(a,await readSale(a));
...
function install(){
  ...
  const section=document.createElement('section'); section.className='owner-card'; section.id='prelaunchDeliveryControl'; section.innerHTML=`
    <h2>Public launch clock + presale distribution</h2><p>This production-only tool locks public launch DAY 0 on-chain, then safely delivers standard presale allocations from day 21 and Buy + Stake allocations with their fixed 5% bonus from day 36. Deterministic receipt PDAs make reruns idempotent.</p>
    <div class="danger"><strong>Mark public launch only on the real public token-launch day.</strong> That one transaction starts the buyer release clocks and the founder 365-day lock. It cannot be reset.</div>
    <div class="owner-actions"><button class="btn btn-secondary" id="preDeliveryConnect">Connect owner</button><button class="btn btn-secondary" id="preMarkPublicLaunch">Mark public launch DAY 0</button></div>
    <label>Final delivery manifest v2</label><input id="preDeliveryFile" type="file" accept="application/json,.json"/>
