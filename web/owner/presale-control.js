...
async function connectAndSign(operation, payload = {}) {
  ...
  return { wallet, operation, payload, timestamp, nonce, message, signature: toBase64(signature) };
}

async function ownerAction(operation, payload = {}) {
  const signed = await connectAndSign(operation, payload);
  return fetchJson('/api/presale/owner', {
    method: 'POST',
...
function fmtRlya(base, max = 2) {
  const n = BigInt(base || 0); const unit = 1_000_000_000n;
  const whole = n / unit; let frac = (n % unit).toString().padStart(9, '0').slice(0, max).replace(/0+$/, '');
...
async function refreshSummary() {
  try {
    ...
    set('preAccess', String(state.access || '--').toUpperCase());
    set('prePrice', fmtPrice(state.currentPriceMicroUsdc));
    set('preTotal', `${fmtRlya(state.totalAllocatedBase)} RLYA`);
    set('preWeb', `${fmtRlya(state.webAllocatedBase)} RLYA`);
    set('preManual', `${fmtRlya(state.manualAllocatedBase)} RLYA`);
    set('preUsdc', `${fmtUsdc(state.totalUsdcRaisedBase)} USDC`);
    set('preReferral', `${fmtUsdc(state.totalReferralUsdcPaidBase)} USDC`);
...
async function lookupBuyer() {
  ...
  if (!result.allocations.length) {
    out.textContent = 'No confirmed allocation for this wallet.';
    return;
  }
...
function install() {
  ...
  section.innerHTML = `
    ...
    <div class="owner-grid" style="margin:14px 0">
      ...
      <div><span>USDC verified</span><strong id="preUsdc">--</strong></div>
      <div><span>Website allocation</span><strong id="preWeb">--</strong></div>
      <div><span>Private/off-site</span><strong id="preManual">--</strong></div>
      <div><span>Referral USDC</span><strong id="preReferral">--</strong></div>
      <div><span>Token launch</span><strong>SEPARATE</strong></div>
    </div>
  ...
  document.getElementById('preOpen').onclick = () => setAccess('open').catch(err => log(`ERROR: ${err.message}`));
