...
check("if (!isOwnerPath && /Android/i.test(navigator.userAgent))" in site and "Object.defineProperty(window, 'RALYA_WALLET_PROVIDER'" in site and "return trustSolana?.connect ? trustSolana : connectorProvider;" in site, 'Trust Wallet Android DApp browser is not isolated to its native Solana transaction provider')
check("if (window.RALYA_WALLET_PROVIDER?.connect) return window.RALYA_WALLET_PROVIDER;" in prelaunch, 'working generic Wallet Standard provider path changed; Phantom/Solflare regression risk')
...
print('Trust Wallet Android uses its native Solana provider without changing Phantom/Solflare routing')
