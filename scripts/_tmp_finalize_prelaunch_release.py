...
# 1) Keep Anchor delivery account parsing comfortably below Solana's stack limit.
...
# 2) Make the standalone localhost harness resolve dependencies from /tmp itself,
# and remove the duplicate final mint variable from the extended test.
...
old_mint = "const mint=await getMint(c,rlya);assert.equal(mint.supply,HARD);assert.equal(mint.mintAuthority,null);assert.equal(mint.freezeAuthority,null);"
if "const finalMint=await getMint(c,rlya)" not in s:
    before, sep, tail = s.rpartition(old_mint)
    if not sep:
        raise SystemExit("final duplicate mint assertion not found")
    new_mint = "const finalMint=await getMint(c,rlya);assert.equal(finalMint.supply,HARD);assert.equal(finalMint.mintAuthority,null);assert.equal(finalMint.freezeAuthority,null);"
    s = before + new_mint + tail
p.write_text(s)

# 3) Reservations affect the live quote price/preview, while confirmed totals remain separate.
replace_once(
    "netlify/functions/_shared/presale-core.mts",
...
replace_once(
    ...
    "atomic owner nonce claim",
)

# 5) Replace the timing-based mutation lock with atomic conditional writes.
p = Path("netlify/functions/_shared/presale-core.mts")
s = p.read_text()
old_lock = """const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));
...
"""
new_lock = """const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));
...
if old_lock in s:
    ...
    raise SystemExit("old mutation lock block not found")
p.write_text(s)

# 6) Atomically claim buyer quote-authorization nonce too.
replace_once(
    ...
    "atomic quote auth claim",
)

# 7) Public state includes active reservations so displayed price matches the next lockable position.
replace_once(
    "netlify/functions/presale-state.mts",
    "    const state = await computeState(store(), false);",
    "    const state = await computeState(store(), true);",
    "public reservation-aware state",
)

# 8) Buyer-specific allocation history is private to the wallet owner.
Path("netlify/functions/presale-wallet.mts").write_text("""import bs58 from 'bs58';
...
# 9) Browser: reservation-aware preview + short-lived signed wallet ownership proof for allocation history.
p = Path("web/prelaunch.js")
s = p.read_text()
if "let allocationViewAuth = null;" not in s:
    s = s.replace("let refreshTimer = null;", "let refreshTimer = null;\nlet allocationViewAuth = null;", 1)
if "function allocationViewMessage(" not in s:
    anchor = """function quoteAuthMessage(walletAddress, usdcAmount, referrer, timestamp, nonce) {
  return [
...
"""
    addition = anchor + """function allocationViewMessage(walletAddress, timestamp, nonce) {
  return [
    'RALYA allocation view',
    `Wallet: ${walletAddress}`,
    `Timestamp: ${timestamp}`,
...
"""
    if anchor not in s:
        raise SystemExit("quote message anchor missing")
...
start = s.index("async function refreshWallet() {")
end = s.index("async function connectWallet() {", start)
new_refresh = """async function signedAllocationViewBody() {
...
old_connect = """    const result = await provider.connect(); wallet = new PublicKey(result?.publicKey || provider.publicKey);
    $$('.wallet-button').forEach(btn => btn.textContent = shorten(wallet.toBase58()));"""
new_connect = """    const result = await provider.connect(); wallet = new PublicKey(result?.publicKey || provider.publicKey);
    if (allocationViewAuth?.wallet !== wallet.toBase58()) allocationViewAuth = null;
    $$('.wallet-button').forEach(btn => btn.textContent = shorten(wallet.toBase58()));"""
if old_connect in s:
    s = s.replace(old_connect, new_connect, 1)
elif new_connect not in s:
    raise SystemExit("connect wallet anchor missing")
p.write_text(s)

# 10) Audit the newly hardened guarantees as permanent release invariants.
p = Path("scripts/audit_prelaunch.py")
s = p.read_text()
core_marker = "check('QUOTE_CONFIRMATION_GRACE_MS' in core, 'quote reservation is not retained through confirmation grace')\n"
core_extra = """check('getWithMetadata' in core and 'onlyIfNew: true' in core and 'onlyIfMatch: current.etag' in core and 'MUTATION_LOCK_LEASE_MS = 120_000' in core, 'financial mutation lock is not atomic/lease-safe')
...
"""
if core_extra not in s:
    if core_marker not in s:
        raise SystemExit("audit core marker missing")
    s = s.replace(core_marker, core_marker + core_extra, 1)
wallet_marker = "check('paymentReference' not in wallet, 'private owner payment references leak through buyer wallet endpoint')\n"
wallet_extra = """check("req.method !== 'POST'" in wallet and 'verifySignature' in wallet and 'RALYA allocation view' in wallet and 'Allocation-view wallet signature verification failed.' in wallet, 'buyer allocation history is not wallet-signature protected')
...
"""
if wallet_extra not in s:
    if wallet_marker not in s:
        raise SystemExit("audit wallet marker missing")
    s = s.replace(wallet_marker, wallet_marker + wallet_extra, 1)
p.write_text(s)

# Final deterministic presence checks before the expensive CI gates.
assert Path("programs/rlya_sale/src/lib.rs").read_text().count("Box<Account<'info, PrelaunchMetrics>>") >= 2
assert "npm install --prefix /tmp" in Path("scripts/local_validator_smoke.sh").read_text()
assert "const finalMint=await getMint(c,rlya)" in Path("scripts/local_validator_smoke.sh").read_text()
assert "onlyIfMatch: current.etag" in Path("netlify/functions/_shared/presale-core.mts").read_text()
assert "RALYA allocation view" in Path("netlify/functions/presale-wallet.mts").read_text()
print("RALYA_TEMP_FINAL_HARDENING_PATCH=PASS")
