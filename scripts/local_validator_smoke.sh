...
# RALYA local-validator protocol integration test.
...
avm install 1.0.2
avm use 1.0.2
# Anchor v1 targets Solana 3.x; pin the project-declared/recommended version
# instead of whatever the moving quick installer currently labels stable.
...
if [[ $deploy_status -ne 0 ]]; then
  echo '[ERROR] Local validator deployment failed; recovery material withheld.' >&2
  grep -E '^Error:|Program log:|Program .* failed|invalid account data|insufficient funds|RPC' /tmp/ralya-local-deploy.log || true
  exit "$deploy_status"
fi
...
let guardCount=0;
async function fail(label,fn,afterCheck=null){try{await fn()}catch{if(afterCheck)await afterCheck();guardCount++;console.log(`GUARD PASS: ${label}`);return}throw new Error(`expected failure: ${label}`)}
...
function quote(s,usdc){let rem=BigInt(usdc),p=s.sold,out=0n,loops=0;while(rem>0n){assert(p<s.presale);assert(++loops<=256);const idx=p/s.step,pr=s.base+idx*s.inc,next=MathMinBig((idx+1n)*s.step,s.presale),avail=next-p,cost=ceil(avail*pr,R);if(rem>=cost){out+=avail;p+=avail;rem-=cost}else{const part=rem*R/pr;assert(part>0n&&part<=avail);out+=part;p+=part;rem=0n}}return out}
const MathMinBig=(a,b)=>a<b?a:b;

const treasury=Keypair.generate(),founder=Keypair.generate(),buyer=Keypair.generate(),buyer2=Keypair.generate(),referrer=Keypair.generate(),manual=Keypair.generate(),attacker=Keypair.generate(),impostor=Keypair.generate();
const fundTx=new Transaction();for(const k of [treasury,founder,buyer,buyer2,referrer,manual,attacker,impostor])fundTx.add(SystemProgram.transfer({fromPubkey:admin.publicKey,toPubkey:k.publicKey,lamports:3*LAMPORTS_PER_SOL}));
...
const fr=(await getOrCreateAssociatedTokenAccount(c,admin,rlya,founder.publicKey)).address;
...
const iu=(await getOrCreateAssociatedTokenAccount(c,admin,usdc,impostor.publicKey)).address;
...
let s=await getState(sale);assert.equal(s.status,1);assert.equal(s.bps,100n);console.log('ACTIVATE PASS');
...
await fail('referred wallet cannot bypass referral with direct buy',()=>send(ix('buy',[m(buyer2.publicKey,true,true),m(a2),m(rlya),m(usdc),m(sale,false,true),m(treasury.publicKey),m(b2u,false,true),m(tu,false,true),m(b2r,false,true),m(sv,false,true),m(TOKEN_PROGRAM_ID)],1n*U,1n),[buyer2]));
...
assert.equal(await amt(b2u),b2u0-gross);assert.equal(await amt(ru),ru0+5n*U);assert.equal(await amt(tu),tu1+495n*U);assert.equal(await amt(b2r),b2r0+q2);console.log('REFERRAL_BUY PASS: 500 USDC -> 5 referrer + 495 treasury');
...
const [attRef]=PublicKey.findProgramAddressSync([referralSeed,attacker.publicKey.toBuffer()],PROGRAM_ID);
await fail('self-referral rejected',()=>send(ix('register_referral',[m(attacker.publicKey,true,true),m(attacker.publicKey),m(attRef,false,true),m(attRef),m(SystemProgram.programId)]),[attacker]));
await fail('two-wallet circular referral rejected',()=>send(ix('register_referral',[m(referrer.publicKey,true,true),m(buyer2.publicKey),m(arref,false,true),m(a2),m(SystemProgram.programId)]),[referrer]));

s=await getState(sale);const oneQ=quote(s,1n*U),slipBuyer0=await amt(bu),slipTreasury0=await amt(tu);
await fail('minimum-output/slippage protection',()=>send(ix('buy',[m(buyer.publicKey,true,true),m(bref),m(rlya),m(usdc),m(sale,false,true),m(treasury.publicKey),m(bu,false,true),m(tu,false,true),m(br,false,true),m(sv,false,true),m(TOKEN_PROGRAM_ID)],1n*U,oneQ+1n),[buyer]),async()=>{assert.equal(await amt(bu),slipBuyer0);assert.equal(await amt(tu),slipTreasury0)});
await fail('wrong treasury identity rejected',()=>send(ix('buy',[m(buyer.publicKey,true,true),m(bref),m(rlya),m(usdc),m(sale,false,true),m(impostor.publicKey),m(bu,false,true),m(iu,false,true),m(br,false,true),m(sv,false,true),m(TOKEN_PROGRAM_ID)],1n*U,1n),[buyer]));
await fail('unauthorized admin pause rejected',()=>send(ix('pause',[m(attacker.publicKey,true),m(rlya),m(sale,false,true)]),[attacker]));

await send(ix('pause',[m(admin.publicKey,true),m(rlya),m(sale,false,true)]));
await fail('purchase rejected while paused',()=>send(ix('buy',[m(buyer.publicKey,true,true),m(bref),m(rlya),m(usdc),m(sale,false,true),m(treasury.publicKey),m(bu,false,true),m(tu,false,true),m(br,false,true),m(sv,false,true),m(TOKEN_PROGRAM_ID)],1n*U,1n),[buyer]));
await send(ix('resume',[m(admin.publicKey,true),m(rlya),m(sale,false,true)]));
console.log('PAUSE_RESUME PASS');
await fail('presale cap cannot be exceeded',()=>send(ix('manual_sale',[m(admin.publicKey,true),m(manual.publicKey),m(rlya),m(sale,false,true),m(sv,false,true),m(mr,false,true),m(TOKEN_PROGRAM_ID)],PRESALE)));
await fail('purchase below 1 USDC minimum rejected',()=>send(ix('buy',[m(buyer.publicKey,true,true),m(bref),m(rlya),m(usdc),m(sale,false,true),m(treasury.publicKey),m(bu,false,true),m(tu,false,true),m(br,false,true),m(sv,false,true),m(TOKEN_PROGRAM_ID)],500_000n,1n),[buyer]));
await fail('founder cannot release before 365-day lock',()=>send(ix('release_founder',[m(founder.publicKey,true,true),m(rlya),m(fl,false,true),m(fv,false,true),m(fr,false,true),m(TOKEN_PROGRAM_ID)]),[founder]));
...
s=await getState(sale);assert.equal(s.raised,600n*U);assert.equal(s.refPaid,5n*U);assert.equal(s.sold,q+q2+man);assert.equal(s.manual,man);assert(guardCount>=10);
